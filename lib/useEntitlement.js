...
import { scheduleTrialEndingNotice } from '../notifications';
...
//
// `has_premium` / `dev_trial_days_left` AsyncStorage overrides are honored
// ONLY when this build is dev or beta — checked here at read time, not just
// at write time. AsyncStorage survives the TestFlight-beta → App Store
// update, so a persisted flag must never grant access in production.
const DEV_OVERRIDES_ENABLED = __DEV__ || process.env.EXPO_PUBLIC_IS_BETA === 'true';
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (status.periodType === 'TRIAL' && status.expirationDate) {
        ...
        // Honest trial-end heads-up (idempotent — stable identifier).
        // Real trials only; the dev simulator path above never reaches here.
        scheduleTrialEndingNotice(days);
      } else {
        setTrialDaysLeft(null);
      }
    } catch (e) {
